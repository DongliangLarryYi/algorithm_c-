...
int solution(long N)
{
    // store some initialized data
    unsigned long long x = 1, y = 1, a = 0, b = 1,t;
    
    // if N equals zeor, will return 0, otherwise will run below code
    while (N>0)
    {
        //decide whether the last digit is zero or not
        if (N & 1)
        {
        ...
        // Below equations are also based on fabonacci identical equations, please refer to https://en.wikipedia.org/wiki/Fibonacci_number
        t = 2 * x*y - x*x;
        y = x*x + y*y;
        x = t;
        
        // make sure all digits were visited
        N >>= 1;
    }
    //find the last six digits
    int last_6digits = a%1000000;
    return last_6digits;
    
}


int solution2(long N)
{
    // store some initialized data
    unsigned long long x = 1, y = 1, a = 0, b = 1,t;
    ...
    while (N>0)
    {
        ...
        {
            //Below equations are based on fabonacci identical equations, please refer to https://en.wikipedia.org/wiki/Fibonacci_number
            t = (a*x%1000000 + b*y%1000000);
            a = (x*b%1000000 - x*a%1000000 + a*y%1000000);
            b = t;
        }
        
        // Below equations are also based on fabonacci identical equations, please refer to https://en.wikipedia.org/wiki/Fibonacci_number
        t = (2 * x*y%1000000 - x*x%1000000);
        y = (x*x + y*y)%1000000;
        x = t;
